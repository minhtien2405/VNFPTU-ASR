...
with mlflow.start_run(run_name="wav2vec2_finetune_central_vi"):
	...
	wer_history = [(log["step"], log["eval_wer"]) for log in trainer.state.log_history if "eval_wer" in log]
	with open("./logs/wav2vec2-base-central-vi/wer_history.json", "w") as f:
		json.dump(wer_history, f)
	mlflow.log_artifact("./logs/wav2vec2-base-central-vi/wer_history.json", artifact_path="wer_history")
