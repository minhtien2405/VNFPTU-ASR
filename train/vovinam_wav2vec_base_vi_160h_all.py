...
def download_audio_from_s3(url: str, cache_dir: str = "./audio_cache") -> Optional[str]:
    ...
    try:
        ...
        temp_path = (
            os.path.join(cache_dir, filename + ".tmp")
...
def load_audio_from_path(audio_path: str, target_sr: int = 16000) -> Optional[Dict]:
    ...
    try:
        ...
        return {"array": audio_array, "sampling_rate": target_sr, "path": audio_path}
    except Exception as e:
        logging.getLogger(__name__).error(
            f"Error loading audio from {audio_path}: {str(e)}"
        )
        return None
...
def validate_audio(sample, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        if audio_data is None:
            logger.warning("No audio data found in sample")
            return False

        if audio_data["array"] is None or len(audio_data["array"]) == 0:
            logger.warning("Invalid audio array in sample")
            return False
        ...
        if duration < 0.1 or duration > 30:  # 0.1s to 30s reasonable range
            logger.warning(
                f"Audio duration {duration:.2f}s is outside reasonable range"
            )
            return False
...
def validate_text(sample, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        if not text or len(text) == 0:
            logger.warning("Empty text found in sample")
            return False
...
def process_audio_sample(sample, logger: logging.Logger) -> Dict:
    ...
    try:
        ...
        logger.error(
            f"Error processing audio sample with audioLink {sample.get('audioLink', 'unknown')}: {str(e)}"
        )
        return None
...
def load_and_prepare_data(config: TrainingConfig, logger: logging.Logger):
    ...
    try:
        ...
        def download_and_prepare_sample(sample):
            ...
            audio_path = download_audio_from_s3(
                sample["audioLink"], cache_dir=os.path.join(config.cache_dir, "audio")
            )
            if not audio_path:
                logger.warning(
                    f"Failed to download audio, skipping. URL: {sample['audioLink']}"
                )
            ...
            try:
                ...
                if duration < 0.1 or duration > 30:
                    logger.warning(
                        f"Invalid duration ({duration:.2f}s), skipping. Path: {audio_path}"
                    )
                    sample["is_valid"] = False
            except Exception as e:
                logger.warning(
                    f"Could not get duration, skipping. Path: {audio_path}, Error: {e}"
                )
                sample["is_valid"] = False
            ...
            if not normalized_text or len(normalized_text.strip()) < 2:
                logger.warning(
                    f"Invalid or empty text ('{text}'), skipping. Path: {audio_path}"
                )
                sample["is_valid"] = False
        ...
        for split in dataset.keys():
            ...
            logger.info(
                f"Filtered {split} split: {original_size} -> {filtered_size} samples."
            )
            if filtered_size == 0 and original_size > 0:
                raise ValueError(
                    f"No valid samples remained in '{split}' split after filtering. Check logs for warnings."
                )
            ...
            columns_to_remove = [
                col
                for col in ["audio", "audioLink", "is_valid", "__index_level_0__"]
                if col in filtered_split.column_names
            ]
...
def prepare_dataset(batch, processor: Wav2Vec2Processor, logger: logging.Logger):
    """
    Prepare dataset for training by explicitly loading audio from path.
    """
    try:
        ...
        logger.error(
            f"Error processing file {batch.get('audio_path', 'UNKNOWN')}: {str(e)}"
        )
        return None
...
def main():
    ...
    try:
        ...
        wandb.init(
            project=config.project_name, name=config.run_name, config=vars(config)
        )
        ...
        train_dataset, valid_dataset, test_dataset = load_and_prepare_data(
            config, logger
        )
        logger.info("Dataset loading and preparation completed.")

        processor, model, metric, compute_metrics = setup_training_components(
            config, logger
        )
        ...
        wer_history_path = os.path.join(
            config.output_dir, "vovinam_wav2vec2_base_vi_160hfinetuned_wer_history.json"
        )
