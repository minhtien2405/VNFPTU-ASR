...
from transformers import WhisperProcessor, WhisperForConditionalGeneration, Seq2SeqTrainingArguments, Seq2SeqTrainer, TrainerCallback, BitsAndBytesConfig
import numpy as np
import json
import logging
import peft 
...
# Load environment variables and login to Hugging Face Hub
...
def filter_long_audio(example):
    ...
    audio_length = example["audio"]["array"].shape[0] / example["audio"]["sampling_rate"]   
    if audio_length > 30:
        num_of_long_audio += 1
        logging.warning(f"Audio {example['audio']['path']} is too long: {audio_length:.2f} seconds")
...
logging.info(f"Sample audio: {train_dataset[0]['audio']['array'][:5]}... (first 5 samples)")
...
model = WhisperForConditionalGeneration.from_pretrained(
	model_id, use_cache=False, device_map="auto",
...
peft_model.model.model.encoder.conv1.register_forward_hook(lambda module, input, output: output.requires_grad_(True))  # re-enable grad computation for conv layer
...
training_args = Seq2SeqTrainingArguments(
    ...
    gradient_accumulation_steps=8,  # effectively batch size of 8
    learning_rate=1e-5, 
    warmup_steps=100,
...
with mlflow.start_run(run_name="phowhisper_finetune_central_vi"):
    ...
    try: 
        ...
        logging.info(f"Final evaluation WER in validation set: {eval_results['eval_wer']}")
        ...
        mlflow.log_artifact("./logs/phowhisper-large-central-vi/eval_results.json", artifact_path="eval_results")
    ...
    # # Log model and processor to MLflow
