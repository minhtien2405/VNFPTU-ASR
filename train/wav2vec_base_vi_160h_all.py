...
class TrainingConfig:
    model_id: str = "khanhld/wav2vec2-base-vietnamese-160h"
    hub_model_id: str = "minhtien2405/wav2vec2-base-vi-160h-finetuned"
    dataset_id: str = "nguyendv02/ViMD_Dataset"
    output_dir: str = "./logs/wav2vec2-base-vi-160h"
    cache_dir: str = "./cache"
    log_dir: str = "./logs"
    model_save_dir: str = "./models/wav2vec2-base-vi-160h"
    ...
    fp16: bool = True
    project_name: str = "Wav2Vec2_160h_All_ViMD_FPTU"
    run_name: str = (
        f"wav2vec2_finetune_vi_160h_all_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
...
def setup_logging(config: TrainingConfig) -> logging.Logger:
    ...
    logging.basicConfig(
        filename=os.path.join(config.log_dir, "wav2vec_base_vi_250h_v0.log"),
        level=logging.INFO,
...
def validate_audio(sample, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        logger.error(
            f"Error validating audio sample {sample.get('path', 'unknown')}: {str(e)}"
        )
        return False
...
def validate_text(sample, logger: logging.Logger) -> bool:
    ...
    try:
        text = sample["text"]
        if (
            not text or len(text) == 0 or len(text) > 1000
        ):  # Adjust max length as needed
            logger.warning(
                f"Invalid text sample: {sample.get('path', 'unknown')}, text: {text}"
            )
            return False
        return True
    except Exception as e:
        logger.error(
            f"Error validating text sample {sample.get('path', 'unknown')}: {str(e)}"
        )
        return False
...
def main():
    ...
    try:
        ...
        wer_history_path = os.path.join(
            config.output_dir, "wav2vec_base_all_vi_160h_wer_history.json"
        )
        ...
        trainer.push_to_hub(
            commit_message="Fine-tuned Wav2Vec2_160h on ViMD All region with layer freezing",
            tags=["speech-recognition", "vietnamese", "vietnam"],
