...
logging.basicConfig(
    filename=os.path.join(log_dir, "eval_phowhisper_large_vi.log"),
    level=logging.INFO,
...
if not hf_token:
    ...
    raise ValueError("Hugging Face token not found. Please set HF_TOKEN in ./configs/.env")
...
# Define WER transformation pipeline
...
def load_dataset(region="Central"):
    """
    ...
    try:
        dataset = hugDS.load_dataset("nguyendv02/ViMD_Dataset", split="test", cache_dir=cache_dir)
...
PEFT_MODEL_ID = "minhtien2405/phowhisper-large-central-vi"
try:
    BASE_MODEL_ID = PeftConfig.from_pretrained(PEFT_MODEL_ID, cache_dir=cache_dir).base_model_name_or_path
...
try:
    ...
    TOKENIZER = WhisperTokenizer.from_pretrained(BASE_MODEL_ID, cache_dir=cache_dir,
    ...
    BASE_MODEL.config.forced_decoder_ids = TOKENIZER.get_decoder_prompt_ids(language="vi", task="transcribe")
    logging.info(f"Feature extractor, tokenizer, and base model loaded successfully.")
    
    MODEL = PeftModel.from_pretrained(
        BASE_MODEL,
        PEFT_MODEL_ID,
        cache_dir=cache_dir
    ).merge_and_unload(progressbar=True)  # Merge LoRA weights to reduce inference latency
except Exception as e:
...
BATCH_SIZE = 16  # Adjust based on hardware: 16 for T4 GPU (float16), 32 for medium model, 48 for small model
...
def evaluate(dataset):
    ...
    for i in tqdm(range(0, len(dataset), BATCH_SIZE), desc="Evaluating"):
        ...
        try:
            ...
            outputs = PIPE(inputs)
            logging.info(f"Batch {i // BATCH_SIZE + 1} output type: {type(outputs)}, sample: {outputs[:2]}")
            
            for j, output in enumerate(outputs):
                ref_text = batch["text"][j]
                hyp_text = output if isinstance(output, str) else output.get("text", "")
                logging.info(f"Sample {i + j}: Reference: {ref_text}, Hypothesis: {hyp_text}")
                if not hyp_text:
                    logging.warning(f"Empty hypothesis for sample {i + j} (filename: {batch['filename'][j]}), skipping.")
                    continue
                try:
                    wer = jiwer.wer(
                        ref_text,
                    ...
                    logging.error(f"Error computing WER for sample {i + j} (filename: {batch['filename'][j]}): {str(e)}")
                    continue
        except Exception as e:
...
def save_results(results, output_file="eval_results.txt"):
    ...
    # Compute overall WER and standard deviation
    ...
    # Compute WER by province_name and gender
    province_wers = {}
...
if __name__ == "__main__":
    # Load dataset
    try:
        dataset = load_dataset(region="Central")
    ...
    # Save results
    save_results(results, output_file="eval_results_central.txt")
